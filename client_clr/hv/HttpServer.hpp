...
public:
    ...
    void registerHttpService(HttpService* service) {
        internals::service = service;
    }

    void setHost(const char* host = "0.0.0.0") {
        if (host) strcpy(internals::host, host);
    }

    void setPort(int port = 0, int ssl_port = 0) {
        if (port != 0) internals::port = port;
        if (ssl_port != 0) internals::https_port = ssl_port;
    }

    void setProcessNum(int num) {
        internals::worker_processes = num;
    }

    void setThreadNum(int num) {
        internals::worker_threads = num;
    }
